...
def login():
    ...
    if form.validate_on_submit():
        ...
        temp_code = TemporaryCode(user_id=user.id, phone=user.phone, code=mfa_code)
...
def mfa():
    ...
    if form.validate_on_submit():
        ...
        temp_code = (
            TemporaryCode.query.filter_by(user_id=user_id)
            .order_by(TemporaryCode.created_at.desc())
            .first()
        )

        if (
            temp_code
            and temp_code.code == pin
            and temp_code.expires_at > datetime.now()
        ):
            session["authenticated"] = True
